...
accessFn start end increment
    | start >= (end + increment) = []
    | otherwise = (!! start):(accessFn (start+increment) end increment)

applyFunction (fn,arg) = fn $ arg
-- map applyFunction (zip (accessFn 0 20 1) numGrid )