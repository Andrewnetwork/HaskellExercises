...
map _ Nil = Nil
map fn (a :. ax) =  fn a :. map fn ax
-- :t ((flatten .) .) (\a -> a*2)
-- :t ((flatten .) .) id (1 :. 2 :. 3 :. 4 :. 5 :. Nil)
...
filter _ Nil = Nil
filter predicate (a :. ax) = if predicate a then a :. filter predicate ax else filter predicate ax
...
-- :t ( (flatten .) .) (\a-> a*2) (\a -> ((1:.Nil) :. Nil)) (\a -> ((1:.Nil) :. Nil))
...
-- flattenAgain ( (1 :. 2 :. 3 :. Nil) :. (1 :. 2 :. 3 :. Nil) :. Nil )
...
seqOptional (Full a :. ax) = case seqOptional ax of Empty -> Empty
                                                    Full b -> Full (a :. b)
